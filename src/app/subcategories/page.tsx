...
export default function SubcategoriesPage() {
  ...
  return (
    <div className="space-y-12">
      ...
      {/* Subcategory Tabs */}
      <div>
        <Text className="text-gr-muted mb-3">Select a subcategory</Text>
      ...
      {/* Leader Highlight */}
      <Card className="bg-gradient-to-r from-gr-success to-gr-raised border-gr-success ring-0 p-8">
        <div className="flex flex-col md:flex-row md:items-center gap-6">
          <div className="flex-1">
            ...
            <h2 className="text-2xl md:text-3xl font-bold text-gr-text mb-3">
              ...
              <span className="text-gr-success">{leader?.count}</span> {formatSubcategory(selectedSubcat).toLowerCase()}
            </h2>
            <p className="text-gr-muted">
              Market average is <span className="text-gr-text">{marketAvg}</span> products.
              {leader && (
                <span>
                  {' '}That&apos;s <span className="text-gr-success">{Math.round((leader.count / marketAvg) * 100)}%</span> of
                  the market average.
                </span>
              )}
            </p>
          ...
          <div className="text-center md:text-right">
            <Metric className="text-gr-success text-5xl font-bold">
              {totalInSubcat}
            </Metric>
            <Text className="text-gr-subtle">total across brands</Text>
          </div>
        </div>
      </Card>
      ...
      <div>
        ...
        <Card className="bg-gr-surface border-gr-border ring-0">
          ...
          <div className="mt-4 flex items-center gap-2 text-sm">
            <div className="w-8 border-t-2 border-dashed border-gr-subtle" />
            <span className="text-gr-subtle">Market average: {marketAvg} products</span>
          </div>
        </Card>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {leaders.map((l) => {
            ...
            return (
              <Card
                key={l.slug}
                className={`ring-0 text-center ${
                  isAboveAvg
                    ? 'bg-gradient-to-br from-gr-success to-gr-raised border-gr-success'
                    : 'bg-gr-surface border-gr-border'
                }`}
              >
                <Text className="text-gr-subtle text-sm truncate">{l.brand}</Text>
                <div
                  className={`text-3xl font-bold mt-2 ${
                    isAboveAvg ? 'text-gr-success' : 'text-gr-subtle'
                  }`}
                >
                  {index}
                </div>
                <Text className="text-gr-subtle text-xs mt-1">
                  {l.count} products
                </Text>
              </Card>
            );
            ...
            return (
              <button
                ...
                className={`text-left p-4 rounded-lg border transition-all ${
                  selectedSubcat === subcat.id
                    ? 'bg-gr-success border-gr-success'
                    : 'bg-gr-surface border-gr-border hover:border-gr-border'
                }`}
              >
                <Text className="text-gr-subtle text-sm">{total} products</Text>
                <h3 className="text-gr-text font-semibold mt-1">{subcat.label}</h3>
                <div className="mt-3 flex justify-between items-center">
                  <span className="text-gr-subtle text-sm">Leader:</span>
                  <span className="text-gr-success text-sm font-medium">
                    {top?.brand} ({top?.count})
                  </span>
                </div>
              </button>
      ...
      <Card className="bg-gr-surface border-gr-border ring-0">
        ...
        <div className="space-y-3 text-gr-muted">
          <p>
            <span className="text-gr-text">Index = (Brand Count / Market Average) × 100.</span>{' '}
            An index of 150 means the brand has 50% more products than average in that subcategory.
          </p>
          <p>
            <span className="text-gr-text">High index = strategic focus.</span>{' '}
            Vuori&apos;s high shorts index (156%) signals this as a key product category for them.
          </p>
          <p>
            <span className="text-gr-text">Low index = potential gap.</span>{' '}
