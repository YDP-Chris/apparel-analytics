...
export default function SubcategoriesPage() {
  ...
  return (
    <div className="space-y-12">
      ...
      {/* Subcategory Tabs */}
      <div>
        <Text className="text-gray-400 mb-3">Select a subcategory</Text>
      ...
      {/* Leader Highlight */}
      <Card className="bg-gradient-to-r from-violet-900/20 to-violet-900/5 border-violet-800/50 ring-0 p-8">
        <div className="flex flex-col md:flex-row md:items-center gap-6">
          <div className="flex-1">
            ...
            <h2 className="text-2xl md:text-3xl font-bold text-white mb-3">
              ...
              <span className="text-violet-400">{leader?.count}</span> {formatSubcategory(selectedSubcat).toLowerCase()}
            </h2>
            <p className="text-gray-400">
              Market average is <span className="text-white">{marketAvg}</span> products.
              {leader && (
                <span>
                  {' '}That&apos;s <span className="text-violet-400">{Math.round((leader.count / marketAvg) * 100)}%</span> of
                  the market average.
                </span>
              )}
            </p>
          ...
          <div className="text-center md:text-right">
            <Metric className="text-violet-400 text-5xl font-bold">
              {totalInSubcat}
            </Metric>
            <Text className="text-gray-500">total across brands</Text>
          </div>
        </div>
      </Card>
      ...
      <div>
        ...
        <Card className="bg-gray-900 border-gray-800 ring-0">
          ...
          <div className="mt-4 flex items-center gap-2 text-sm">
            <div className="w-8 border-t-2 border-dashed border-gray-500" />
            <span className="text-gray-500">Market average: {marketAvg} products</span>
          </div>
        </Card>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {leaders.map((l) => {
            ...
            return (
              ...
              >
                <Text className="text-gray-400 text-sm truncate">{l.brand}</Text>
                <div
                  className={`text-3xl font-bold mt-2 ${
                    isAboveAvg ? 'text-violet-400' : 'text-gray-500'
                  }`}
                >
                  {index}
                </div>
          ...
          {KEY_SUBCATEGORIES.map((subcat) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="text-white font-semibold mt-1">{subcat.label}</h3>
      ...
      <Card className="bg-gray-900 border-gray-800 ring-0">
        ...
        <div className="space-y-3 text-gray-400">
          <p>
            <span className="text-white">Index = (Brand Count / Market Average) × 100.</span>{' '}
            An index of 150 means the brand has 50% more products than average in that subcategory.
          </p>
          <p>
            <span className="text-white">High index = strategic focus.</span>{' '}
            Vuori&apos;s high shorts index (156%) signals this as a key product category for them.
          </p>
          <p>
            <span className="text-white">Low index = potential gap.</span>{' '}
