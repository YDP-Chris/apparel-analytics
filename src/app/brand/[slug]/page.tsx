  ...
  return (
    <div className="space-y-12">
      ...
      <div>
        <Link
          href="/brands"
          className="text-gr-subtle hover:text-gr-accent transition-colors text-sm"
        >
      ...
      <div className="flex flex-col md:flex-row md:items-end gap-6">
        <div className="flex-1">
          ...
          <h1 className="text-4xl font-bold text-gr-text mb-2">{brand.name}</h1>
          <p className="text-gr-muted text-lg">
        ...
        <div className="flex gap-8">
          <div className="text-center">
            <Metric className="text-gr-accent text-4xl font-bold">
              {brand.total.toLocaleString()}
            </Metric>
            <Text className="text-gr-subtle">Products</Text>
          </div>
          <div className="text-center">
            <Metric className="text-gr-text text-4xl font-bold">#{rank}</Metric>
            <Text className="text-gr-subtle">Rank</Text>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="bg-gr-surface border-gr-border ring-0">
          <Text className="text-gr-subtle">Categories</Text>
          <div className="text-2xl font-bold text-gr-text mt-1">
            {Object.keys(brand.categories).filter((k) => brand.categories[k] > 0).length}
          </div>
        </Card>
        <Card className="bg-gr-surface border-gr-border ring-0">
          <Text className="text-gr-subtle">Subcategories</Text>
          <div className="text-2xl font-bold text-gr-text mt-1">
            {Object.keys(brand.subcategories).filter((k) => brand.subcategories[k] > 0).length}
          </div>
        </Card>
        <Card className="bg-gr-surface border-gr-border ring-0">
          <Text className="text-gr-subtle">Top Category</Text>
          <div className="text-2xl font-bold text-gr-text mt-1">
            {formatCategory(
        ...
        <Card className="bg-gr-surface border-gr-border ring-0">
          <Text className="text-gr-subtle">Bottoms %</Text>
          <div className="text-2xl font-bold text-gr-text mt-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Category Breakdown */}
        <Card className="bg-gr-surface border-gr-border ring-0">
          <h3 className="text-gr-text font-semibold text-lg mb-4">
            Category Breakdown
          </h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Strengths */}
        <Card className="bg-gradient-to-br from-gr-success to-gr-raised border-gr-success ring-0">
          <h3 className="text-gr-text font-semibold text-lg mb-4">
              ...
              {strongSubcats.map((s) => (
                <div key={s.subcat}>
                  <div className="flex justify-between items-center">
                    <span className="text-gr-text">{formatSubcategory(s.subcat)}</span>
                    <span className="text-gr-success font-bold">{s.index}%</span>
                  </div>
                  <div className="flex justify-between text-sm text-gr-subtle mt-1">
        ...
        {/* Gaps */}
        <Card className="bg-gradient-to-br from-gr-accent-soft to-gr-raised border-gr-accent-soft ring-0">
          <h3 className="text-gr-text font-semibold text-lg mb-4">
          ...
          {gaps.length > 0 ? (
            <div className="space-y-4">
              {gaps.map((g) => (
                <div key={g.subcat}>
                  <div className="flex justify-between items-center">
                    <span className="text-gr-text">{formatSubcategory(g.subcat)}</span>
                    <span className="text-gr-accent font-bold">{g.index}%</span>
                  </div>
                  <div className="flex justify-between text-sm text-gr-subtle mt-1">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gr-border">
                <th className="text-left py-3 px-4 text-gr-muted font-medium">
                  Subcategory
                </th>
                <th className="text-right py-3 px-4 text-gr-muted font-medium">
                  Products
                </th>
                <th className="text-right py-3 px-4 text-gr-muted font-medium">
                  % of Catalog
            ...
            <tbody>
              {Object.entries(brand.subcategories)
                ...
                .map(([subcat, count]) => {
                  ...
                  return (
                    <tr key={subcat} className="border-b border-gr-border">
                      <td className="py-3 px-4 text-gr-text">
                        {formatSubcategory(subcat)}
                      </td>
                      <td className="py-3 px-4 text-right text-gr-muted">
                        {count}
                      </td>
                      <td className="py-3 px-4 text-right text-gr-subtle">
                        {pct}%
                      </td>
                      <td className="py-3 px-4 text-right text-gr-subtle">
                        {avg}
